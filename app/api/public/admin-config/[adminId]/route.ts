...
export async function GET(req: NextRequest, { params }: { params: Promise<{ adminId: string }> }) {
    try {
        ...
        // Optional: Check if admin exists to show admin's name on form header
        const admin = await User.findById(adminId).lean();
        if (!admin || admin.role !== 'admin') {
            return NextResponse.json({ error: 'Invalid Organization ID' }, { status: 404 });
        }
        ...
        return NextResponse.json({ 
            success: true, 
            config,
            adminEmail: admin.email // Just a piece of info for UI
        });
    } catch (error: any) {

...
import { Plus, Calendar, MapPin, ExternalLink, Loader2, Star } from 'lucide-react';
...
export default function SpecialEventsPage() {
    ...
    const fetchEvents = async () => {
        setLoading(true);
        try {
            const res = await fetch('/api/events?type=special');
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-foreground flex items-center gap-2">
                        <Star className="w-8 h-8 text-purple-600 fill-purple-100" />
                        Special Events
                    </h1>
                    <p className="text-muted-foreground mt-1">Manage your single-day events, functions, and symposiums.</p>
                </div>
            ...
            {showForm && (
                <div className="bg-card border border-border p-6 rounded-xl shadow-sm animate-in fade-in slide-in-from-top-4">
                    <h2 className="text-lg font-semibold mb-4 text-card-foreground">Create New Special Event</h2>
                    <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="col-span-2 md:col-span-1">
                            ...
                            <input
                                ...
                                placeholder="e.g. Hostel Day 2026"
                                value={formData.name}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {events.length === 0 && (
                        <div className="col-span-full text-center py-12 border-2 border-dashed border-border rounded-xl bg-muted/5">
                            ...
                            <p className="text-muted-foreground mt-1">Get started by creating your first special event.</p>

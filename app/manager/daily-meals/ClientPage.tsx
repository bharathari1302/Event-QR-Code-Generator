...
import { Plus, Calendar, MapPin, ExternalLink, Loader2, Coffee } from 'lucide-react';
...
export default function DailyMealsPage() {
    ...
    const [formData, setFormData] = useState({ name: '', date: '', venue: '', eventType: 'daily' as 'special' | 'daily' });
    const [creating, setCreating] = useState(false);

    const fetchEvents = async () => {
        setLoading(true);
        try {
            const res = await fetch('/api/events?type=daily');
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                setFormData({ name: '', date: '', venue: '', eventType: 'daily' });
                setShowForm(false);
                fetchEvents();
            }
        } catch (error) {
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-foreground flex items-center gap-2">
                        <Coffee className="w-8 h-8 text-blue-600 fill-blue-100" />
                        Daily Meals
                    </h1>
                    <p className="text-muted-foreground mt-1">Manage continuous hostel mess facilities and daily scan logs.</p>
                </div>
                <Button onClick={() => setShowForm(!showForm)}>
                    <Plus className="w-4 h-4 mr-2" />
                    Create Daily Facility
                </Button>
            </div>
            ...
            {showForm && (
                <div className="bg-card border border-border p-6 rounded-xl shadow-sm animate-in fade-in slide-in-from-top-4">
                    <h2 className="text-lg font-semibold mb-4 text-card-foreground">Create New Daily Mess Facility</h2>
                    <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="col-span-2 md:col-span-1">
                            ...
                            <input
                                ...
                                placeholder="e.g. Even Semester Mess 2026"
                                value={formData.name}
                        ...
                        <div className="col-span-2">
                            ...
                            <input
                                type="text"
                                className="w-full p-2.5 bg-background border border-input rounded-md focus:ring-2 focus:ring-ring focus:outline-none text-foreground"
                                placeholder="e.g. Boys Mess Block B"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {events.length === 0 && (
                        <div className="col-span-full text-center py-12 border-2 border-dashed border-border rounded-xl bg-muted/5">
                            ...
                            <h3 className="text-lg font-medium text-foreground">No daily meals recorded</h3>
                            <p className="text-muted-foreground mt-1">Get started by setting up a continuous facility.</p>
